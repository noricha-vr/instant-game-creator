...
移行時は`LOCAL_GAMES_FILE`の`canvas-worker-sim-v1`生成物を別途棚卸しし、既存`/g/:slug`を維持します。移行が完了するまでは旧JSONを読み取り専用で残し、新しいHTMLアプリの保存処理からは書き込みません。

## 6. 注意点

- 生成HTMLは保存前と読取時にCSP注入を行い、禁止トークン検査を通す
