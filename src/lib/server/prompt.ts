...
- アイデアに通信・複数人・複数デバイス・ログイン・データ永続化が含まれる場合、その体験の核を一人・1画面・保存なしで再現する翻案をカードにする（例: 対戦→CPU対戦や交互プレイ、チャット→AI風キャラとの疑似チャット、SNS→架空タイムライン）。
- 単一HTMLで完結する方向だけにする。`;
}
...
- 説明文は禁止。
- JSONオブジェクトだけを返す。
- JSONのキーは必ず title, summary, howToUse, html, adaptation の5つ。
- title は30文字以内の日本語。
- summary は80文字以内の日本語。
- howToUse は使い方を示す日本語文字列の配列。1〜4件。
- html は完全な単一HTML文書。500〜30000文字。
- adaptation は string または null。翻案した場合だけ1行で説明する。翻案不要なら null。

HTMLアプリの要件:
- 入力が通信・複数人・複数デバイス・ログイン・永続保存を前提とする場合、その体験の核を一人・1画面・セッション内だけで再現する形に翻案する。黙って劣化版を作らない。翻案した場合は adaptation に1行で説明を書く（例: 「対戦相手はCPUにアレンジしました」）。
